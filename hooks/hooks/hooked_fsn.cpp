...
void remove_smoke( ) {
	static auto smoke_count = *reinterpret_cast< uint32_t ** >( util::pattern_scan( "client.dll", "A3 ? ? ? ? 57 8B CB" ) + 1 );
...
void __stdcall hooks::hooked_fsn( ClientFrameStage_t stage ) {
	static auto original_fn = client_hook->get_func_address<FrameStageNotify_t>( 36 );
	...
	else if ( !g_csgo.m_clientstate( )->m_net_channel && netchannel_hook ) {
		netchannel_hook->clear_class_base( );
		delete netchannel_hook;
		netchannel_hook = nullptr;
	}
	...
	if ( stage == FRAME_RENDER_START ) {
		...
		bullettracers::get().draw_beam();

		update_spoofer( );

		nightmode::get().dark_mode();
		...
		if (g_ctx.available() && g_ctx.m_local && g_ctx.m_local->is_alive())
		{
		...
		if ( g_ctx.m_local->is_alive( ) ) {
			...
			g_ctx.m_local->get_animlayers()[12].m_flWeight = 0.f;
			g_ctx.m_local->set_abs_angles(vec3_t(0.f, g_ctx.m_local->get_animation_state()->m_goal_feet_yaw, 0.f));
