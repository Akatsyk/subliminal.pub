...
IVModelInfo* C_CSGO::m_modelinfo( ) {
	if ( !p_modelinfo )
		p_modelinfo = get_interface<IVModelInfo>( "engine.dll", "VModelInfoClient" );
...
IViewRenderBeams * C_CSGO::m_viewrenderbeams( ) {
	if ( !p_viewrenderbeams )
		p_viewrenderbeams = *reinterpret_cast< IViewRenderBeams ** >( util::pattern_scan( "client.dll", "A1 ? ? ? ? 56 8B F1 B9 ? ? ? ? FF 50 08" ) + 1 );
...
CGlowObjectManager * C_CSGO::m_glow( ) {
	if ( !p_glow )
		p_glow = *reinterpret_cast< CGlowObjectManager** >( util::pattern_scan( "client.dll", "0F 11 05 ? ? ? ? 83 C8 01" ) + 3 );
...
IMoveHelper * C_CSGO::m_movehelper( ) {
	if ( !p_movehelper )
		p_movehelper = **reinterpret_cast< IMoveHelper*** >( util::pattern_scan( "client.dll", "8B 0D ? ? ? ? 8B 46 08 68" ) + 2 );
...
CInput * C_CSGO::m_input( ) {
	if ( !p_input )
		p_input = *reinterpret_cast< CInput** >( util::pattern_scan( "client.dll", "B9 ? ? ? ? F3 0F 11 04 24 FF 50 10 " ) + 0x1 );
...
C_CSPlayerResource * C_CSGO::m_playerresource( ) {
	if ( !p_playerresource )
		p_playerresource = **reinterpret_cast< C_CSPlayerResource *** >( util::pattern_scan( "client.dll", "8B 3D ? ? ? ? 85 FF 0F 84 ? ? ? ? 81 C7" ) + 2 );
...
DWORD C_CSGO::m_postprocessing( ) {
	if ( !p_postprocessing )
		p_postprocessing = *( DWORD* )( util::pattern_scan( "client.dll", "80 3D ? ? ? ? ? 53 56 57 0F 85" ) + 2 );
...
DWORD C_CSGO::m_ccsplayerrenderablevftable( ) {
	if ( !p_ccsplayerrenderablevftable )
		p_ccsplayerrenderablevftable = *( DWORD * )( util::pattern_scan( "client.dll", "55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 89 7C 24 0C" ) + 0x4E );
