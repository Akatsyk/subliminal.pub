...
void weapon_t::update_accuracy_penality() {
	call_virtual<void(__thiscall*)(void*)>(this, 471)(this);
}
...
bool weapon_t::is_rifle( ) {
	switch ( get_csweapon_info( )->m_weapon_type) {
...
void player_t::SetPoseParameters(float* poses) {
	std::memcpy(m_flPoseParameter(), poses, sizeof(float) * 24);
}

void player_t::GetPoseParameters(float* poses) {
...
CUserCmd*& player_t::m_pCurrentCommand( ) {
	static auto current_command = *( uint32_t* )( util::pattern_scan( "client.dll", "89 BE ? ? ? ? E8 ? ? ? ? 85 FF" ) + 2 );

	return *( CUserCmd ** )( ( uintptr_t )this + current_command );
}

void player_t::invalidate_bone_cache( ) {
	static auto addr = ( DWORD )util::pattern_scan( "client.dll", "C6 05 ? ? ? ? ? 89 47 70" + 2);
...
void player_t::set_abs_origin( const vec3_t & origin ) {
	...
	static auto fn = reinterpret_cast< Fn >( util::pattern_scan( "client.dll", "55 8B EC 83 E4 F8 51 53 56 57 8B F1" ) );
	fn( this, origin );
}
...
void player_t::set_abs_angles( const vec3_t & angle ) {
	...
	static auto fn = reinterpret_cast< Fn >( util::pattern_scan( "client.dll", "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1 E8" ) );
	fn( this, angle );
}

void player_t::standard_blending_rules(CStudioHdr* hdr, Vector* pos, Quaternion* q, float curTime, int boneMask)
{
	using Fn = void(__thiscall*)(void*, CStudioHdr*, Vector*, Quaternion*, float, int);

	static auto fn = reinterpret_cast<Fn>(util::pattern_scan("client.dll", "55 8B EC 83 E4 F0 B8 ? ? ? ? E8 ? ? ? ? 56 8B 75 08 57 8B F9 85 F6"));
	fn(this, hdr, pos, q, curTime, boneMask);
}

void player_t::build_transformations(CStudioHdr* hdr, Vector* pos, Quaternion* q, const matrix3x4_t& cameraTransform, int boneMask, byte* boneComputed)
{
	using Fn = void(__thiscall*)(void*, CStudioHdr*, Vector*, Quaternion*, matrix3x4_t const&, int, byte*);

	static auto fn = reinterpret_cast<Fn>(util::pattern_scan("client.dll", "55 8B EC 83 E4 F0 81 EC ? ? ? ? 56 57 8B F9 8B 0D ? ? ? ? 89 7C 24 1C"));
	fn(this, hdr, pos, q, cameraTransform, boneMask, boneComputed);
}

void player_t::set_anim_layers(std::array<AnimationLayer, 15>& layers)
{
	auto this_layers = this->get_anim_layers_();
	std::copy(std::begin(this_layers), std::end(this_layers), std::begin(layers));
}

void player_t::set_pose_params(std::array<float, 24>& params)
{
...
int player_t::animlayer_count( ) {
	return *( int* )( ( DWORD )this + 0x2970 + 12);
}
...
int player_t::sequence_activity( int sequence ) {
	...
	static auto sequence_activity = reinterpret_cast<int( __fastcall* )( void*, studiohdr_t*, int )>( util::pattern_scan( "client.dll", "53 56 8B F1 8B DA 85 F6 74 55" ) );
...
c_baseplayeranimationstate * player_t::get_animation_state( ) {
	return *reinterpret_cast< c_baseplayeranimationstate ** >( reinterpret_cast< void * >( uintptr_t( this ) + 0x3874) );
}
...
CBoneAccessor * player_t::m_BoneAccessor( ) {
	return reinterpret_cast< CBoneAccessor* >( uintptr_t( this ) + 0x2698 );
}
