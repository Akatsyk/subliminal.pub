...
#define NETVARA(type, name, table, netvar, add)                           \
    type& name##() const {                                          \
        static int _##name = netvars::get( ).get_offset(table, netvar);     \
        return *(type*)((uintptr_t)this + _##name + add);                 \
	}
...
public:
	...
	NETVARA(float, m_flOldSimulationTime, "CBaseEntity", "m_flSimulationTime", 4);
	...
	NETVAR( vec3_t, m_angNetworkAngles, "CBaseEntity", "m_angNetworkAngles");
	ANETVAR(float, get_pose_params, 24, "CBaseAnimating", "m_flPoseParameter")
	...
	APOFFSET(get_anim_layers_, AnimationLayer, 15, 0x2970)

	void invalidate_bone_cache( );
	void set_abs_origin( const vec3_t & angle );
	void set_abs_angles( const vec3_t & angle );
	void standard_blending_rules(CStudioHdr* hdr, Vector* pos, Quaternion* q, float curTime, int boneMask);
	void build_transformations(CStudioHdr* hdr, Vector* pos, Quaternion* q, const matrix3x4_t& cameraTransform, int boneMask, byte* boneComputed);
	void set_anim_layers(std::array<AnimationLayer, 15>& layers);
	...
	int& m_fEffects() {
		return *reinterpret_cast<int*>(uintptr_t(this) + 0xEC);
	}
};
