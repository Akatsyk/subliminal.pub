...
#define clamp(val, min, max) (((val) > (max)) ? (max) : (((val) < (min)) ? (min) : (val)))
	...
	if (local_weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER) {
		...
		if (flPostponeFireReady && flPostponeFireReady < g_csgo.m_globals()->m_curtime + g_csgo.m_globals()->m_interval_per_tick * g_csgo.m_clientstate()->m_choked_commands && g_cfg.ragebot.autoshoot) {
			g_ctx.get_command()->m_buttons &= ~IN_ATTACK;
...
void aimbot::iterate_players() {
	...
	for (int i = 1; i <= g_csgo.m_globals()->m_maxclients; i++) {
		...
		if (location == vec3_t(0, 0, 0) || !location.IsValid() )
			continue;
...
void aimbot::aim() {
	...
	//apply record while backtrack turned on
	if (target_data.backtrack && target_data.best_record)
		target_data.best_record->apply(e);

	backup.apply(e, true);


	bool hitchanced = hitchance(next_angle, e, config.hitchance_amount);
	...
	if (!hitchanced)
	{
		...
		if (config.autoscope && !g_ctx.m_local->m_bIsScoped())
		{
			if (g_ctx.get_command()->m_buttons & IN_ATTACK)
				g_ctx.get_command()->m_buttons &= ~IN_ATTACK;
...
aimbot::bt_point_return_t aimbot::best_point_backtrack( player_t* e, player_record_t& player ) {
	...
	for (int i = 0; i < track.size(); i++) {
		...
		std::vector< int > hitboxes = hitboxes_from_vector(player.m_e, record.m_flags, record.m_velocity, config.hitscan_history);
...
bool aimbot::hitchance(vec3_t angles, player_t* e, float chance) {
	...
	local_weapon->update_accuracy_penality();
	float
	...
	for (int i = 0; i < 256; i++) {
		...
		view_forward = src + (view_forward * local_weapon->get_csweapon_info()->m_range);

		trace_t tr;
		Ray_t ray;

		ray.Init(src, view_forward);
		g_csgo.m_trace()->ClipRayToEntity(ray, MASK_SHOT_HULL | CONTENTS_HITBOX, e, &tr);
...
bool aimbot::head_check(player_t* e, int m_fFlags, vec3_t m_vecVelocity) {
	...
	if (g_cfg.ragebot.antiaim_correction || config.baim[BAIM_NOTRESOLVED].enabled) {
		...
		if (player.m_resolver->m_mode != lby_update || player.m_resolver->m_mode != lby)
			use = false;
	}
