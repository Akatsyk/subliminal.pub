...
void antiaim::create_move() {
	...
	if (m_weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER) {
		...
		if (m_weapon->get_csweapon_info()->m_weapon_type == WEAPONTYPE_KNIFE && ((g_ctx.get_command()->m_buttons & IN_ATTACK) || (g_ctx.get_command()->m_buttons & IN_ATTACK2))) {
			return;
		}
	...
	if (m_bBreakLowerBody && !g_csgo.m_clientstate()->m_choked_commands) {
...
void antiaim::update_lowerbody_breaker() {
	...
	float
		...
		speed = animation_state::get().animstate()->m_speed;
