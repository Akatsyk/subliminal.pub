...
autowall::returninfo_t autowall::calculate_return_info( vec3_t start, vec3_t end, IClientEntity * from_entity, IClientEntity * to_entity, int specific_hitgroup ) {
	...
	while ( fire_bullet_data.m_current_damage > 0 ) {
		return_info.m_penetration_count = fire_bullet_data.m_penetration_count;

		trace_line( fire_bullet_data.m_current_position, end, MASK_SHOT, from_entity, &fire_bullet_data.m_enter_trace );
		clip_trace_to_player( fire_bullet_data.m_current_position, fire_bullet_data.m_current_position + ( fire_bullet_data.m_direction * 40.f ), e_to, MASK_SHOT, fire_bullet_data.m_filter, &fire_bullet_data.m_enter_trace );
...
void autowall::scale_damage( player_t * e, csweaponinfo_t * weapon_info, int hitgroup, float & current_damage ) {
	...
	if ( armor_value > 0 && is_armored( ) ) {
		float bonus_value = 1.f, armor_bonus_ratio = 0.5f, armor_ratio = weapon_info->m_armor_ratio / 2.f;
...
bool autowall::handle_bullet_penetration( csweaponinfo_t * info, firebulletdata_t & data ) {
	...
	float lost_damage = fmaxf(
		...
		+ ( ( data.m_current_damage * final_damage_modifier )
			+ ( fmaxf( 3.75 / info->m_penetration, 0.f ) * 3.f * modifier ) ), 0.f );
...
bool autowall::is_breakable_entity( IClientEntity * e ) {
	...
	static auto fn = reinterpret_cast< Fn >( util::pattern_scan( "client.dll", "55 8B EC 51 56 8B F1 85 F6 74 68 83 BE") );
