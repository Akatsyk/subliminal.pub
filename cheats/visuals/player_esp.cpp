...
void playeresp::paint_traverse() {
	...
	for ( int i = 1; i < g_csgo.m_globals()->m_maxclients; i++ ) {
		...
		Box box; if ( util::get_bbox( e , box ) ) {
			...
			vec3_t screen;
			if (math::world_to_screen(aimbot::get().target_data.aimspot, screen))
				render::get().rect_filled(screen.x, screen.y, 12, 12, Color(255, 0, 255, 255));

			if ( g_cfg.player.skeleton [ SKELETON_BACKTRACK ].enabled ) {
				Color color = e->IsDormant() ? Color( 130 , 130 , 130 , 130 ) : Color::White;
				color.SetAlpha( g_cfg.player.alpha  * esp_alpha_fade [ e->EntIndex() ] );

				draw_skeleton_backtrack( e , color );
			}
			if (g_cfg.player.skeleton[SKELETON_BACKTRACK].enabled) {
				Color color = e->IsDormant() ? Color(130, 130, 130, 130) : Color::White;
				color.SetAlpha(g_cfg.player.alpha * esp_alpha_fade[e->EntIndex()]);
...
void playeresp::draw_ammobar( player_t* m_entity , Box box ) {
	...
	auto data = weapon->get_csweapon_info();
	if (!data)
		return;
	...
	float bar_width = ammo * box.w / data->m_max_clip1;
	...
	if ( weapon->m_iClip1() != weapon->get_csweapon_info()->m_max_clip1 )
...
void playeresp::draw_skeleton_backtrack( player_t * e , Color color ) {
	auto& player = lagcompensation::get().players[e->EntIndex()];
	...
	auto matrix = track.at(0).m_bone_matrix;
