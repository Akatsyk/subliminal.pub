...
void engineprediction::start( ) {
	...
	orig_currtime = g_csgo.m_globals()->m_curtime;
	orig_frametime = g_csgo.m_globals()->m_frametime;

	local_player->m_pCurrentCommand( ) = g_ctx.get_command( );

	if ( !m_random_seed || !m_prediction_player ) {
		m_random_seed = *reinterpret_cast< int ** >( util::pattern_scan( "client.dll", "A3 ? ? ? ? 66 0F 6E 86" ) + 1 );
		m_prediction_player = *reinterpret_cast< int ** >( util::pattern_scan( "client.dll", "89 35 ? ? ? ? F3 0F 10 48" ) + 2 );
	}
	...
	*m_prediction_player = uintptr_t( local_player );
	...
	g_csgo.m_globals( )->m_frametime = local_player->m_fFlags() & FL_FROZEN ? 0.f :  g_csgo.m_globals( )->m_interval_per_tick;
...
void engineprediction::finish() const
{
	g_ctx.m_local->m_nTickBase() = m_tickbase_backup;
	g_ctx.m_local->m_pCurrentCommand() = nullptr;

	g_csgo.m_gamemovement()->FinishTrackPredictionErrors(g_ctx.m_local);
	g_csgo.m_movehelper()->SetHost(nullptr);

	*m_random_seed = -1;
	*m_prediction_player = 0;

	g_csgo.m_globals()->m_curtime = orig_currtime;
	g_csgo.m_globals()->m_frametime = orig_frametime;
	g_ctx.m_local->m_fFlags() = m_flags_backup;
}
