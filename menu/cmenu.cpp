...
void menu::render( ) {
	...
	auto outline = [ & ] ( int offset, Color color ) {
		rect_t m_pos = rect_t(
			area.x - 75, area.y - 22, area.w + 75, area.h + 22
		);
	...
	auto top_bar = [ this ] ( ) -> void {
		...
		render::get( ).rect_filled( top_area.x, top_area.y, top_area.w, top_area.h + 2, Color( 29, 29, 29 ) );
		render::get().rect(top_area.x, top_area.y, top_area.w + 1, top_area.h + 3, Color(49, 49, 49));

		//colored line
		render::get().rect_filled(top_area.x + 1, top_area.y + 2, top_area.w - 1, 1, g_cfg.menu.menu_theme[0]);

		render::get().rect_filled(top_area.x, top_area.y + 22, top_area.w + 1, 1, Color(49, 49, 49));
	...
	render::get( ).rect_filled( area.x, area.y, area.w, area.h, Color( 24, 24, 24 ) );

	render::get( ).rect_filled( main_tab_area.x, main_tab_area.y, main_tab_area.w, main_tab_area.h - 19, Color( 30, 30, 30 ) );

	for ( int i = 0; i < tabs.size( ); i++ ) {
		tabs[ i ]->paint( );
	}

	outline( 0, Color(49, 49, 49) );

	//bottom bar
	render::get().rect_filled(area.x - 75, area.y + area.h - 19, area.w + 76, 20, Color(29, 29, 29));
	render::get().rect(area.x - 75, area.y + area.h - 19, area.w + 76, 20, Color(49, 49, 49));
