...
C_GroupBox::C_GroupBox( GroupRow row, int y, std::string n_name, int sub_tab_count ) {
	...
	int tab_size = menu::get().area.w / 2 - 23;

	int x = 8;
	if ( row == GROUP_RIGHT )
		x += tab_size + 20;

	area = rect_t( x, y, tab_size, menu::get( ).area.h - 35 );
...
void C_GroupBox::render( ) {
	render::get( ).rect_filled( area.x, area.y, area.w + 10, area.h, Color( 29, 29, 29 ) );
	render::get( ).rect( area.x, area.y, area.w + 10, area.h, Color( 49, 49, 49) );
}
