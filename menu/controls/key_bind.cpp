...
void C_KeyBind::draw( ) {
	...
	if ( item_region.contains_point( mouse ) ) {
		render::get( ).rect_filled( n_area.x, n_area.y, n_area.w, 20, Color { 30, 30, 30 } );
	}
	else
		render::get( ).rect_filled( n_area.x, n_area.y, n_area.w, 20, Color { 24, 24, 24 } );

	render::get().rect(n_area.x, n_area.y, n_area.w, 20, Color{ 49,49,49 });
