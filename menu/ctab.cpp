...
void C_Tab::paint( ) {
	...
	render::get().rect(tab_area.x, tab_area.y, 2, tab_area.h + 1, Color::Black);
	if ( index == menu::get( ).selected_tab ) {
		render::get().rect(tab_area.x, tab_area.y, 2, tab_area.h + 1, g_cfg.menu.menu_theme[0]);
		render::get( ).text( fonts[ TAHOMA12 ], tab_area.x + 8, tab_area.y + 10, Color::White, HFONT_CENTERED_Y, name.c_str( ) );
		paint_subtabs( );
	}
	render::get().rect(tab_area.x, tab_area.y, tab_area.w, tab_area.h + 1, Color(49, 49, 49));
}
